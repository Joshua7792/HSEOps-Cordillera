...
SCANNED_PDF_MESSAGE = (
    "This PDF appears to be scanned/image-only. Install OCR dependencies "
    "or provide a text-based PDF."
)
OCR_DARK_CELL = "__OCR_DARK_CELL__"
...
def extract_additional_training_page(page: pdfplumber.page.Page) -> dict[str, dict[str, date]]:
    ...
    return extract_additional_training_words(words)


def extract_additional_training_words(words: list[dict]) -> dict[str, dict[str, date]]:
    if not words:
        return {}
...
class OCRDependencyError(RuntimeError):
    """Raised when a scanned PDF needs OCR but local OCR support is unavailable."""
...
def _words_to_text(words: list[dict]) -> str:
    ...
    for word in sorted(words, key=lambda w: (w["top"], w["x0"])):
        if not rows or abs(word["top"] - rows[-1]["top"]) > 7:
            rows.append({"top": word["top"], "words": [word]})
        else:
            rows[-1]["words"].append(word)
    return "\n".join(
        " ".join(w["text"] for w in sorted(row["words"], key=lambda w: w["x0"]))
        for row in rows
    )
...
def _ocr_date_tokens(words: list[dict], top_min: float, top_max: float, x_min: float) -> list[dict]:
    data_words = sorted(
        [w for w in words if top_min <= w["top"] <= top_max and w["x0"] >= x_min],
        key=lambda w: (w["top"], w["x0"]),
    )
    tokens: list[dict] = []
    consumed: set[int] = set()

    def add_token(text: str, parts: list[dict]) -> bool:
        cleaned_text = _clean_ocr_token(text)
        dt = parse_date(cleaned_text)
        if not dt:
            dt = parse_date(text.strip())
        if not dt:
            return False
        tokens.append({
            "text": cleaned_text or text.strip(),
    ...
    for i, word in enumerate(data_words):
        if id(word) in consumed:
            continue
        raw = _clean_ocr_token(word["text"])
        if DATE_RE.fullmatch(raw) and add_token(raw, [word]):
            continue

        # OCR sometimes splits "3/31/2026" into adjacent pieces such as
        # "3/31/202" + "6". Try tight same-line joins before giving up.
        parts = [word]
        joined = raw
        prev = word
        for next_word in data_words[i + 1:i + 4]:
            if id(next_word) in consumed:
                continue
            if abs(next_word["top"] - word["top"]) > 7:
                continue
            gap = next_word["x0"] - prev["x1"]
            if gap < -2 or gap > 12:
                continue
            parts.append(next_word)
            joined += _clean_ocr_token(next_word["text"])
            prev = next_word
            if DATE_RE.fullmatch(joined) and add_token(joined, parts):
                break
    ...
    year_re = re.compile(r"\d{4}")
    for month_word in data_words:
        if id(month_word) in consumed:
            continue
        month_text = normalize(month_word["text"].rstrip(".,"))
        if month_text not in month_names:
            continue
        month_cx, _ = _word_center(month_word)
        best_year = None
        best_score = float("inf")
        for year_word in data_words:
            if id(year_word) in consumed:
                continue
            if not year_re.fullmatch(_clean_ocr_token(year_word["text"])):
                continue
            year_cx, _ = _word_center(year_word)
            dx = abs(month_cx - year_cx)
            dy = year_word["top"] - month_word["top"]
            if dx > 45 or dy < -5 or dy > 28:
                continue
            score = dx + abs(dy) * 0.5
            if score < best_score:
                best_score = score
        ...
        if best_year is not None:
            add_token(f"{month_word['text']} {best_year['text']}", [month_word, best_year])

    return tokens


def _extract_worker_rows_from_words(
    words: list[dict],
    top_min: float = 195,
    top_max: float = 430,
    x_max: float = 180,
    ...
    name_words = [
        ...
        and w["x0"] <= x_max
        and not DATE_RE.fullmatch(_clean_ocr_token(w["text"]))
    ]
    rows: list[dict] = []
    for word in sorted(name_words, key=lambda w: (w["top"], w["x0"])):
        if not rows or abs(word["top"] - rows[-1]["top"]) > 10:
            rows.append({"top": word["top"], "words": [word]})
        else:
            rows[-1]["words"].append(word)
    ...
    for row in rows:
        row_words = sorted(row["words"], key=lambda w: w["x0"])
        name = _clean_worker_name(" ".join(w["text"] for w in row_words).strip())
        name_norm = normalize(name)
        if not name or "nombre" in name_norm or "empleado" in name_norm:
            continue
        if len(name_norm) < 4:
            continue
        centers = [_word_center(w)[1] for w in row_words]
        worker_rows.append({"name": name, "center_y": sum(centers) / len(centers)})
    return worker_rows


def _clean_worker_name(name: str) -> str:
    cleaned = re.sub(r"^\s*(?:\d+|[IVXLC]+)[\.\)]\s*", "", name.strip(), flags=re.IGNORECASE)
    ...
    parts = cleaned.split()
    if len(parts) >= 2 and parts[-1] in {"uan", "ngel"}:
        parts = [parts[-1], *parts[:-1]]
        cleaned = " ".join(parts)
    for bad_start, fixed_start in (("ngel ", "Angel "), ("uan ", "Juan ")):
        if cleaned.startswith(bad_start):
            cleaned = fixed_start + cleaned[len(bad_start):]
    return cleaned


def _line_runs(flags: list[bool], min_len: int = 1) -> list[tuple[int, int, float]]:
    runs: list[tuple[int, int, float]] = []
    start: Optional[int] = None
    for idx, value in enumerate([*flags, False]):
        if value and start is None:
            start = idx
        elif not value and start is not None:
            if idx - start >= min_len:
                runs.append((start, idx - 1, (start + idx - 1) / 2))
            start = None
    return runs
...
def _merge_positions(values: list[float], tolerance: float) -> list[float]:
    ...
    for value in sorted(values):
        if not merged or abs(value - merged[-1]) > tolerance:
            ...
            merged[-1] = (merged[-1] + float(value)) / 2
    return merged


def _cell_words_to_text(words: list[dict]) -> str:
    rows: list[dict] = []
    for word in sorted(words, key=lambda w: (w["top"], w["x0"])):
        if not rows or abs(word["top"] - rows[-1]["top"]) > 8:
            rows.append({"top": word["top"], "words": [word]})
        else:
            rows[-1]["words"].append(word)
    return "\n".join(
...
def _parse_ocr_date_cell(text: Optional[str], default_month: Optional[int] = None) -> Optional[date]:
    if not text:
        return None
    raw = str(text).replace("\\", "/").replace("\n", " ")
    candidates = [raw, _clean_ocr_token(raw)]

    for token in re.findall(r"[^\sA-Za-z]*\d{1,3}/\d{1,2}/\d{2,4}", raw):
        cleaned = _clean_ocr_token(token.replace("\\", "/"))
        candidates.append(cleaned)
        parts = cleaned.split("/")
        if len(parts) == 3 and parts[0].isdigit() and int(parts[0]) > 12 and parts[0].startswith("1"):
            candidates.append(f"{parts[0][1:]}/{parts[1]}/{parts[2]}")

    if default_month:
        for day, year in re.findall(r"(?<!\d)(\d{1,2})/(\d{2,4})(?!\d)", raw):
            candidates.append(f"{default_month}/{day}/{year}")
...
def _looks_like_ocr_date_noise(text: str) -> bool:
    ...
    if re.search(r"\d", cleaned):
        return True
    known_header_words = {
        "nombre", "empleado", "proteccion", "contra", "caidas", "flama",
        "expuesta", "espacios", "confinados", "manejo", "equipo",
        "motorizado", "excavacion", "zanja", "tijeras", "scissor", "lift",
        "seguridad", "electrica", "lockout", "gruas", "escaleras",
        "andamios", "trabajos", "plomo", "asbesto", "comunicacion",
        "riesgos", "ppe", "adiestramiento",
    }
    words = set(normalize(text).split())
    if words & known_header_words:
        return False
    return len(cleaned) <= 8


def _dominant_ocr_month(words: list[dict]) -> Optional[int]:
    counts: dict[int, int] = {}
    for word in words:
        dt = _parse_ocr_date_cell(word.get("text"))
        if dt:
...
def _dominant_ocr_date(words: list[dict]) -> Optional[date]:
...
def _correct_ocr_date(dt: date, default_dt: Optional[date]) -> date:
    if not default_dt:
        return dt
    if abs((dt - default_dt).days) > 180:
        return default_dt
    if dt.year == default_dt.year and dt.day == default_dt.day and dt.month != default_dt.month:
        return default_dt
    if dt.year == default_dt.year and dt.month == default_dt.month and abs(dt.day - default_dt.day) <= 10:
        return default_dt
    return dt


def _detect_ocr_table_grids(image, page_width: float, page_height: float) -> list[dict[str, list[float]]]:
    gray = image.convert("L")
    pixels = gray.load()
    width, height = image.size
    sx = width / page_width
    sy = height / page_height
    threshold = 150

    horizontal_pixels: list[int] = []
    for y in range(height):
        dark_count = sum(1 for x in range(width) if pixels[x, y] < threshold)
        if dark_count > width * 0.40:
            horizontal_pixels.append(y)

    horizontal_groups: list[list[int]] = []
    for y in horizontal_pixels:
        if not horizontal_groups or y - horizontal_groups[-1][-1] > 3:
            horizontal_groups.append([y])
        else:
            horizontal_groups[-1].append(y)

    horizontal_lines = [sum(group) / len(group) for group in horizontal_groups]
    bands: list[list[float]] = []
    for y in horizontal_lines:
        if not bands or (y - bands[-1][-1]) / sy > 75:
            bands.append([y])
        else:
            bands[-1].append(y)
    ...
    for band in bands:
        if len(band) < 4:
            continue
        y0 = max(0, int(min(band)) - 3)
        y1 = min(height - 1, int(max(band)) + 3)

        active_columns = []
        for x in range(width):
            dark_count = sum(1 for y in range(y0, y1 + 1) if pixels[x, y] < threshold)
            active_columns.append(dark_count > 0)

        active_runs = _line_runs(active_columns)
        if not active_runs:
            continue
        x_start, x_end, _ = max(active_runs, key=lambda run: run[1] - run[0])

        x_min = max(0, x_start - 4)
        x_max = min(width - 1, x_end + 4)
        vertical_flags = []
        for x in range(x_min, x_max + 1):
            dark_count = sum(1 for y in range(y0, y1 + 1) if pixels[x, y] < threshold)
            vertical_flags.append(dark_count > (y1 - y0 + 1) * 0.32)

        x_lines = [x_start / sx]
        x_lines.extend((x_min + run[2]) / sx for run in _line_runs(vertical_flags))
        x_lines.append(x_end / sx)
        x_lines = _merge_positions(x_lines, 3.0)
        y_lines = _merge_positions([y / sy for y in band], 3.0)
        if len(x_lines) >= 3 and len(y_lines) >= 3:
            grids.append({"x": x_lines, "y": y_lines})
    return grids
...
def _ocr_words_to_table(
    ...
    for row_idx in range(len(y_lines) - 1):
        ...
        for col_idx in range(len(x_lines) - 1):
            x0, x1 = x_lines[col_idx], x_lines[col_idx + 1]
            top, bottom = y_lines[row_idx], y_lines[row_idx + 1]
            cell_words = [
                word for word in words
                if x0 - 1 <= _word_center(word)[0] <= x1 + 1
                and top - 1 <= _word_center(word)[1] <= bottom + 1
            ]
            text = _cell_words_to_text(cell_words)
            dt = _parse_ocr_date_cell(text, default_month)
            if dt:
                row.append(f"{dt.month}/{dt.day}/{dt.year}")
                continue
            dark_count = 0
            if not text:
                px0 = max(0, int((x0 + 2) * sx))
                px1 = min(image_width - 1, int((x1 - 2) * sx))
                py0 = max(0, int((top + 2) * sy))
                py1 = min(image_height - 1, int((bottom - 2) * sy))
                if px1 > px0 and py1 > py0:
                    ...
                    if dark_count > 120:
                        text = OCR_DARK_CELL
            elif x0 > page_width * 0.20 and _looks_like_ocr_date_noise(text):
                px0 = max(0, int((x0 + 2) * sx))
                px1 = min(image_width - 1, int((x1 - 2) * sx))
                py0 = max(0, int((top + 2) * sy))
                py1 = min(image_height - 1, int((bottom - 2) * sy))
...
def _table_column_has_dates(table: list[list], col_idx: int, start_idx: int) -> bool:
...
def _table_column_has_training_mark(table: list[list], col_idx: int, start_idx: int) -> bool:
    for row in table[start_idx:]:
        if col_idx < len(row) and (row[col_idx] == OCR_DARK_CELL or parse_date(row[col_idx])):
            return True
    return False


def _ocr_additional_header(label: str) -> str:
    if label == OCR_DARK_CELL:
        return ""
    canonical = canonicalize_page2_header(label)
    if canonical:
        return canonical
    if "ppe" in normalize(label):
        return "PPE"
    return label


def extract_training_table_data(
    table: list[list],
    page_num: int = 0,
    table_idx: int = 0,
    *,
    ocr_mode: bool = False,
    ocr_additional: bool = False,
    ocr_fallback_names: Optional[list[str]] = None,
    ocr_default_date: Optional[date] = None,
    ...
    header_idx = None
    name_col_override: Optional[int] = None
    for i, row in enumerate(table):
        if row and any("empleado" in normalize((c or "")) for c in row):
            header_idx = i
            break
    if header_idx is None and ocr_mode:
        for i, row in enumerate(table):
            if not row:
                continue
            date_cols = [
                idx for idx, cell in enumerate(row)
                if parse_date(str(cell or ""))
            ]
            name_candidates = [
                idx for idx, cell in enumerate(row)
                if cell not in (None, "", OCR_DARK_CELL) and not parse_date(str(cell or ""))
            ]
            if len(date_cols) >= 2 and name_candidates:
                header_idx = max(0, i - 1)
                name_col_override = name_candidates[0]
                break
    ...
    name_col = (
        name_col_override
        if name_col_override is not None
        else next((i for i, h in enumerate(header_row) if "empleado" in normalize(h)), 0)
    )
    first_data_idx = find_first_data_row(table, header_idx, name_col)
    header = build_table_header(table, header_idx, first_data_idx)

    if len(header) < max((len(row or []) for row in table), default=0):
        header.extend([""] * (max(len(row or []) for row in table) - len(header)))

    if ocr_additional:
        for idx, value in enumerate(header):
            if idx != name_col:
                header[idx] = _ocr_additional_header(value)
        if not any(
            idx != name_col and normalize(header[idx])
            for idx in range(len(header))
        ):
            for idx in range(name_col + 1, len(header)):
                if _table_column_has_training_mark(table, idx, first_data_idx):
                    header[idx] = "PPE"
                    break

    cert_cols: list[tuple[int, str]] = []
    baseline_idx = 0
    baseline_order = [name for name, section, _ in REQUIRED_BASELINE_CERTS if section == "HSE Required"]
    for idx, h in enumerate(header):
        if idx == name_col:
            continue
        h_norm = normalize(h)
        if not h_norm:
            if not (ocr_additional and _table_column_has_dates(table, idx, first_data_idx)):
                continue
            h = "PPE"
            h_norm = normalize(h)
        if "nombre" in h_norm or h_norm == "adiestramiento":
            continue
        if ocr_mode and not ocr_additional and baseline_idx < len(baseline_order):
            h = baseline_order[baseline_idx]
        cert_cols.append((idx, h))
        if ocr_mode and not ocr_additional:
            baseline_idx += 1

    if DEBUG:
        print(f"  [debug] page {page_num} table {table_idx}: header_idx={header_idx}")
    ...
    for row in table[first_data_idx:]:
        if not row:
            continue
        row_dates = [
            parse_date(str(cell or ""))
            for cell in row
            if cell != OCR_DARK_CELL and parse_date(str(cell or ""))
        ]
        row_has_training_mark = any(
            idx != name_col and idx < len(row) and row[idx] == OCR_DARK_CELL
            for idx, _header_name in cert_cols
        )
        row_default_date = row_dates[0] if row_dates else (ocr_default_date if row_has_training_mark else None)
        row_has_date = bool(row_default_date)
        raw_worker_name = row[name_col] if name_col < len(row) else ""
        if raw_worker_name == OCR_DARK_CELL:
            raw_worker_name = ""
        if not raw_worker_name and ocr_mode and seen_data and not row_has_date:
            break
        if not raw_worker_name and ocr_fallback_names and fallback_idx < len(ocr_fallback_names):
            raw_worker_name = ocr_fallback_names[fallback_idx]
        if name_col >= len(row) or not raw_worker_name:
            if ocr_mode and seen_data and not row_has_date:
                break
            continue
        worker_name = " ".join(str(raw_worker_name or "").split()).strip()
        if ocr_mode:
            worker_name = _clean_worker_name(worker_name)
        worker_norm = normalize(worker_name)
        if ocr_mode and seen_data and (
            "certifico" in worker_norm
            or "informacion requerida" in worker_norm
            or "lilly del caribe" in worker_norm
        ):
            break
        if not worker_name or "nombre" in worker_norm:
            continue
        entry: dict[str, date] = {}
        for idx, header_name in cert_cols:
            value = value_for_cert_column(row, idx, header_name, header)
            if value == OCR_DARK_CELL and row_default_date:
                dt = row_default_date
            elif ocr_mode:
                default_month = ocr_default_date.month if ocr_default_date else None
                dt = _parse_ocr_date_cell(value, default_month)
            else:
                dt = parse_date(value)
            if ocr_mode and dt:
                dt = _correct_ocr_date(dt, ocr_default_date)
            if dt:
                entry[header_name] = dt
        if entry:
            parsed.setdefault(worker_name, {}).update(entry)
            seen_data = True
        fallback_idx += 1
    return parsed


def _page1_baseline_columns(page_width: float) -> list[dict]:
    # Same Anejo 3 page-1 form layout, expressed as ratios so it survives
    # different render resolutions and minor page-size variation.
    columns = [
        ("Protección Contra Caídas", 0.256),
        ("Flama Expuesta", 0.309),
        ("Espacios Confinados", 0.365),
        ("Manejo de Equipo Motorizado", 0.412),
        ("Excavación o Zanja", 0.474),
        ("Manejo de Tijeras (Scissor Lift)", 0.532),
        ("Seguridad Eléctrica", 0.580),
        ("Lockout", 0.632),
        ("Manejo de Grúas", 0.681),
        ("Escaleras", 0.739),
        ("Andamios", 0.791),
        ("Trabajos con Plomo o Asbesto", 0.846),
        ("Comunicación de Riesgos", 0.908),
    ]
    return [{"name": name, "center_x": page_width * ratio} for name, ratio in columns]


def _page2_default_training_columns(page_width: float) -> list[dict]:
    columns = [
        ("Inducción Jacobs/Lilly", 0.284),
        ("OSHA 10", 0.364),
        ("OSHA 30", 0.432),
        ("Rebar Safety", 0.504),
        ("Formwork & Shoring", 0.560),
        ("Silica Exposure", 0.624),
        ("Concrete & Masonry", 0.697),
        ("Equipment Training", 0.767),
        ("OSHA 30 501", 0.837),
    ]
    return [{"name": name, "center_x": page_width * ratio} for name, ratio in columns]


def extract_baseline_training_words(words: list[dict], page_width: float) -> dict[str, dict[str, date]]:
    worker_rows = _extract_worker_rows_from_words(words)
    if not worker_rows:
        return {}

    column_defs = _page1_baseline_columns(page_width)
    parsed: dict[str, dict[str, date]] = {}
    for token in _ocr_date_tokens(words, top_min=190, top_max=430, x_min=175):
        _, center_y = _word_center(token)
        worker_row = min(worker_rows, key=lambda row: abs(row["center_y"] - center_y))
        if abs(worker_row["center_y"] - center_y) > 24:
            continue
        center_x, _ = _word_center(token)
        cert_col = min(column_defs, key=lambda col: abs(col["center_x"] - center_x))
        if abs(cert_col["center_x"] - center_x) > page_width * 0.04:
            continue
        dt = parse_date(token["text"])
        if dt:
            parsed.setdefault(worker_row["name"], {})[cert_col["name"]] = dt
    return parsed


def extract_additional_training_ocr_words(words: list[dict], page_width: float) -> dict[str, dict[str, date]]:
    parsed = extract_additional_training_words(words)
    if parsed:
        return parsed

    worker_rows = _extract_worker_rows_from_words(words, top_min=195, top_max=450, x_max=180)
    if not worker_rows:
        return {}

    column_defs = _page2_default_training_columns(page_width)
    fallback: dict[str, dict[str, date]] = {}
    for token in _ocr_date_tokens(words, top_min=195, top_max=450, x_min=180):
        _, center_y = _word_center(token)
        worker_row = min(worker_rows, key=lambda row: abs(row["center_y"] - center_y))
        if abs(worker_row["center_y"] - center_y) > 24:
            continue
        center_x, _ = _word_center(token)
        training_col = min(column_defs, key=lambda col: abs(col["center_x"] - center_x))
        if abs(training_col["center_x"] - center_x) > page_width * 0.045:
            continue
        dt = parse_date(token["text"])
        if dt:
            fallback.setdefault(worker_row["name"], {})[training_col["name"]] = dt
    return fallback


def _ocr_pdf_pages(pdf_path: Path) -> list[dict]:
    try:
        import fitz  # PyMuPDF
        import pytesseract
        ...
        raise OCRDependencyError(
            f"{SCANNED_PDF_MESSAGE}\nMissing Python OCR package: {exc.name}."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(f"Could not open PDF for OCR: {pdf_path.name}") from exc
    ...
    try:
        for page_index, page in enumerate(document, start=1):
            ...
            if default_tesseract.exists() and not Path(str(pytesseract.pytesseract.tesseract_cmd)).exists():
                pytesseract.pytesseract.tesseract_cmd = str(default_tesseract)
            try:
                data = pytesseract.image_to_data(image, output_type=Output.DICT)
            except pytesseract.TesseractNotFoundError as exc:
                raise OCRDependencyError(
                    f"{SCANNED_PDF_MESSAGE}\nTesseract OCR executable was not found on PATH."
                ) from exc
            except pytesseract.TesseractError as exc:
                raise OCRDependencyError(
                    f"{SCANNED_PDF_MESSAGE}\nTesseract OCR failed: {exc}"
                ) from exc

            sx = pix.width / float(page.rect.width)
            sy = pix.height / float(page.rect.height)
            words: list[dict] = []
            ...
            default_month = _dominant_ocr_month(words)
            tables = [
                _ocr_words_to_table(
                    words,
                    grid,
                ...
                for grid in _detect_ocr_table_grids(image, float(page.rect.width), float(page.rect.height))
            ]
            pages.append({
                "page_num": page_index,
...
def extract_pdf_data_ocr(
    pdf_path: Path,
) -> tuple[Optional[str], Optional[str], dict[str, dict[str, date]]]:
    contractor: Optional[str] = None
    primary_contact: Optional[str] = None
    workers: dict[str, dict[str, date]] = {}
    previous_baseline_names: list[str] = []

    for page in _ocr_pdf_pages(pdf_path):
        text = page["text"]
        ...
        is_additional = page["page_num"] % 2 == 0 or "nombre del adiestramiento" in page_norm
        page_default_date = _dominant_ocr_date(page["words"])
        page_workers: dict[str, dict[str, date]] = {}
        for table_idx, table in enumerate(page.get("tables", [])):
            merge_worker_certs(
                page_workers,
                extract_training_table_data(
                    table,
                    page["page_num"],
                    table_idx,
                    ocr_mode=True,
                    ocr_additional=is_additional,
                    ocr_fallback_names=previous_baseline_names if is_additional else None,
                    ocr_default_date=page_default_date,
                ),
            )

        if page_workers:
            merge_worker_certs(workers, page_workers)
            if not is_additional:
                previous_baseline_names = list(page_workers)
        elif is_additional:
            merge_worker_certs(
                workers,
                extract_additional_training_ocr_words(page["words"], page["width"]),
            )
        else:
            merge_worker_certs(
                workers,
                extract_baseline_training_words(page["words"], page["width"]),
            )
...
def extract_pdf_data(
    ...
    if extracted_text_chars < 40 or contractor is None or not workers:
        if DEBUG:
            print(
                "  [debug] normal PDF extraction incomplete; "
                f"text_chars={extracted_text_chars}, "
                f"contractor={bool(contractor)}, workers={len(workers)}. Trying OCR fallback."
            )
        ocr_contractor, ocr_contact, ocr_workers = extract_pdf_data_ocr(pdf_path)
        if ocr_contractor:
            contractor = ocr_contractor
        if ocr_contact:
            primary_contact = ocr_contact
        if ocr_workers:
            merge_worker_certs(workers, ocr_workers)
